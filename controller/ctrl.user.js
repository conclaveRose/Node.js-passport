...
const passport = require('passport'); //패스포트 불러온다
require('../controller/passport')(passport); //Passport.js를 불러온다. (passport를 인자로 passport.js파일로 전달하고 그것을 다시 받아온다. )
const User = require('../models/userStorage'); //스키마 모델을 불러온다 ( 이것이 데이터 베이스 이다. 이 객체 안으로 유저의 정보가 담긴다.)
const bcrypt = require('bcryptjs'); //유저의 비밀번호를 해시로 암호화 해준다.
